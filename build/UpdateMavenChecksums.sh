...
#
# Run it after bumping SfmcNativeVersion (or any module version), then diff the result - a hash
# that CHANGED for an unchanged version is exactly the event the pins exist to catch, and wants
# investigating rather than committing.
#
...
# locally, with a mismatch between the two being a hard failure.
set -eu
